...
api.post('/analysis/upload', async (c) => {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .insert({
      ...
      file_name: fileName,
      file_type: fileType,
      file_url: '', // TODO: Supabase Storage 업로드
...
api.post('/strategy/debate', async (c) => {
  ...
  // 디베이트 비동기 실행 (응답은 먼저 반환, 프론트엔드에서 Realtime 구독)
  runDebate(
    {
...
// === 스케줄러 엔드포인트 (Cloud Scheduler에서 호출) ===
const scheduler = new Hono();

// 매일 06:00 — 전체 테넌트 성과 수집
scheduler.post('/collect-metrics', async (c) => {
  ...
  console.log(`[Scheduler] Metrics collected: ${results.length} records`);
